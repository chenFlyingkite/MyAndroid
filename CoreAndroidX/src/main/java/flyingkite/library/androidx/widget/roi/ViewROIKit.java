package flyingkite.library.androidx.widget.roi;
...
public class ViewROIKit implements View.OnTouchListener {
    private static final boolean DEBUG_TOUCH = false;
    private boolean isEnabled;
    private final ViewROIUnit mROIUnit;
    private float SCALE_MIN = 0.05f; // 1/20
    private float SCALE_MAX = 20.0f; // 20
    ...
    public ViewROIKit(Context context, ViewROIUnit roiUnit) {
        mROIUnit = roiUnit;
    ...
    /**
     * Set scale level by setMaxScale(max) and setMinScale(1/max)
     * We will also ensure max >= 1
     */
    public void setScaleLevel(int max) {
        max = MathUtils.clamp(max, 1, Integer.MAX_VALUE);
        setMaxScale(max);
        setMinScale(1F/max);
    }
    ...
    public void showROI(RectF roi) {
        ...
        final float width = mROIUnit.getMovieViewWidth();
        final float height = mROIUnit.getMovieViewHeight();

        float sx = 1F / roi.width(); // sx = scale
        float sy = 1F / roi.height(); // sy = scale, sx & sy should be same
        scale = MathUtils.clamp((sx + sy) / 2, SCALE_MIN, SCALE_MAX); // Use mean if sx != sy (since it may have floating number missing when division)
    ...
    public boolean onTouch(View v, MotionEvent e) {
        ...
        switch (e.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                offset_X = MathUtils.clamp(offset_X, range.left - viewer.x, range.right - viewer.x);

                // Constraint offset_Y to ensure minY <= viewer.y + offset_Y <= maxY
                offset_Y = MathUtils.clamp(offset_Y, range.top - viewer.y, range.bottom - viewer.y);
    ...
    private class ScaleListener extends ScaleGestureDetector.SimpleOnScaleGestureListener {
        ...
        public boolean onScale(ScaleGestureDetector detector) {
            ...
            scale = MathUtils.clamp(scale, SCALE_MIN, SCALE_MAX);
